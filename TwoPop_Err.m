...
% Gridspace
xlim = 10; x0 = -xlim; xf = xlim; dx = 0.1; t0 = 0; tf = 0.2; 
dt = dx*(tf - t0)/(xf - x0); 
...
for k = 1:M
    for j = 1:N
        nn(k,j) = 0.1.*exp(-(x(j) - 0./3).^2./0.5).*sech(x(j) - lambda.*t(k));
%         mm(k,j) = 1 - nn(k,j);
%         mm(k,j) = 0.3.*sech(-(x(j) - x0./115).^2./0.9).*exp(x(j) - lambda.*t(k));
        mm(k,j) = 0.005.*cos(0.01.*pi.*x(j)).*sech(x(j) - lambda.*t(k));
        cc(k,j) = 1.5.*cos(0.11.*pi.*x(j)).*sech(x(j) - lambda.*t(k));
        Er0(k,j) = 0*nn(k,j);
    end
end
...
% -----------------------------------------------------------------------
  ...
  Nf(k,j) = n(j); Mf(k,j) = m(j); Cf(k,j) = c(j);
%   Err(k,j) = Er(j);
    end
end
...
figure
plot(x, Cf(:,100), 'r', x, Cf(:,1500), 'b', x, Cf(:,3000), 'c', x, Cf(:,M), 'y', 'linewidth', 3)
